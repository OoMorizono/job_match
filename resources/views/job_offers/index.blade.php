        ...
        <div class="flex justify-between">
            <div class="w-2/5">
                ...
                <ul>
                    ...
                    <li class="mb-2"><a href="/?{{ http_build_query(array_merge($sort, ['occupation' => $o->id])) }}"
                            class="hover:text-blue-500 {{ strpos(url()->full(), 'occupation=' . $o->id) ? 'text-green-500 font-bold' : '' }}">{{
                            $o->name }}</a></li>
                    </li>
                    @endforeach
                </ul>
                            ...
                            <div class="mt-4 flex items-center space-x-4 py-6">
                                <div>
                                    ...
                                    <img class="h-8 w-8 rounded-full object-cover"
                                        src="{{ $j->company->profile_photo_url }}" alt="{{ $j->company->name }}" />
                                    @endif
                                </div>
                                <div class="text-sm font-semibold">{{ $j->company->name }}<span
                                        class="font-normal ml-2">{{
                                        $j->created_at->diffForHumans() }}</span></div>
                            </div>
                            <div>
