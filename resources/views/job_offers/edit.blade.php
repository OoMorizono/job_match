<x-app-layout>
    <div class="container lg:w-1/2 md:w-4/5 w-11/12 mx-auto mt-8 px-8 bg-indigo-900 shadow-md rounded-md">
        ...
        <form action="{{ route('job_offers.update', $jobOffer) }}" method="POST" class="rounded pt-3 pb-8 mb-4">
            @csrf
            @method('PUT')
            <div class="mb-4">
            ...
            <div class="mb-4">
                ...
                <select name="occupation_id"
                    ...
                    <option value="{{ $occupation->id }}" @if($occupation->id == old('occupation_id',
                        $jobOffer->occupation_id)) selected @endif>{{ $occupation->name }}</option>
                    @endforeach
                </select>
            </div>
            <div class="mb-4">
            ...
            <div class="mb-4">
                ...
                <select name="status"
                    ...
                    <option value="{{ $value }}" @if ($value==old('status', $jobOffer->status)) selected @endif>{{ $name
                        }}</option>
                    @endforeach
                </select>
            </div>
